  ...
  superagent.get(url)
    .then(results => {
      ...
      if (results.body.status !== 'OK' || !isNaN(results.body.results[0].address_components[0].long_name, 10)) {
        ...
        let { hunger, trivia, interest, music, } = request.body;

        let sql = 'SELECT * FROM user_info;';
        client.query(sql)
          .then(results => {
            if (results.rows.length > 0) {
              let sql = 'UPDATE user_info SET lat=$1, long=$2, location=$3, hunger=$4, interest=$5, music=$6, trivia=$7 WHERE user_id IS NOT NULL;';
              let safeValues = [locationObject.lat, locationObject.lng, locationObject.location, hunger, interest, music, trivia];
              client.query(sql, safeValues);
            } else {
              let sql = 'INSERT INTO user_info (lat, long, location, hunger, interest, music, trivia) VALUES ($1, $2, $3, $4, $5, $6, $7);';
              let safeValues = [locationObject.lat, locationObject.lng, locationObject.location, hunger, interest, music, trivia];
              client.query(sql, safeValues);
            }
          })
